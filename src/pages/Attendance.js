...
const Toast = ({ show, message, type, onClose }) => {
  ...
  return (
    <div className={`fixed bottom-4 sm:bottom-6 left-4 right-4 sm:left-1/2 sm:right-auto sm:transform sm:-translate-x-1/2 sm:max-w-md z-50 px-4 sm:px-6 py-3 rounded-lg shadow-lg transition-all duration-300 ${
      type === 'success' 
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={onClose} className="ml-2 opacity-70 hover:opacity-100 p-1">
          <X size={14} />
        </button>
      </div>
    </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4">
      <div className="bg-white rounded-t-xl sm:rounded-xl w-full max-w-md p-4 sm:p-6 shadow-2xl">
        ...
        <p className="text-gray-600 mb-6 leading-relaxed text-sm sm:text-base">{message}</p>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4">
      <div className="bg-white rounded-t-xl sm:rounded-xl w-full max-w-md p-4 sm:p-6 shadow-2xl">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-base sm:text-lg font-semibold text-gray-900">Export Data Presensi ke Excel</h3>
        </div>
...
const StatusButton = React.memo(({ status, active, onClick, icon: Icon, label, disabled = false }) => {
  const getStatusClass = () => {
    const baseClass = "flex items-center justify-center gap-1 px-2 sm:px-3 py-2 sm:py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 border-2 disabled:opacity-50 disabled:cursor-not-allowed min-h-[40px] sm:min-h-auto touch-manipulation";
...
const StatsCard = React.memo(({ icon: Icon, number, label, color }) => {
  ...
  return (
    <div className={`bg-white rounded-lg p-3 sm:p-4 border border-l-4 ${getLeftBorderColor()} border-gray-200 transition-all duration-200 hover:shadow-md hover:-translate-y-1 shadow-sm`}>
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xl sm:text-2xl font-bold mb-1 text-gray-900">{number}</div>
...
const StudentCard = ({ student, originalIndex, attendance, activeClass, updateStatus, updateNote, saving }) => {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3 shadow-sm">
      <div className="flex justify-between items-start">
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-gray-900 text-sm truncate">{student.nama_siswa}</h3>
...
const useAttendance = (currentUser) => {
  ...
  const loadAttendanceForDate = useCallback(async (date, classNum) => {
    try {
        ...
        if (newAttendanceData[classNum]) {
          ...
          if (data && data.length > 0) {
            data.forEach(record => {
              ...
              const studentIndex = studentsData[classNum]?.findIndex(s => s.nisn === record.nisn);
              if (studentIndex !== -1 && newAttendanceData[classNum][studentIndex]) {
...
const Attendance = ({ currentUser = { role: 'admin', kelas: null, username: 'admin' } }) => {
  const {
    studentsData,
    attendanceData,
    setAttendanceData,
  ...
  const filteredStudents = useMemo(() => {
    ...
    return studentsData[activeClass].filter(student =>
  ...
  // Check class access
  const checkClassAccess = useCallback((classNum) => {
  ...
  const updateStatus = useCallback((classNum, studentIndex, status) => {
    if (!checkClassAccess(classNum)) return;

    setAttendanceData(prev => {
      const newAttendanceData = { ...prev };
      if (newAttendanceData[classNum] && newAttendanceData[classNum][studentIndex]) {
        newAttendanceData[classNum][studentIndex] = {
          ...newAttendanceData[classNum][studentIndex],
          status
        };
  ...
  const updateNote = useCallback((classNum, studentIndex, note) => {
    if (!checkClassAccess(classNum)) return;
  ...
  const checkExistingAttendance = useCallback(async (classNum, date) => {
    try {
      const { data, error } = await supabase
        .from('attendance')
        .select('*')
  ...
  const saveAttendanceData = useCallback(async (classNum, date) => {
    try {
      if (!attendanceData[classNum] || Object.keys(attendanceData[classNum]).length === 0) {
        throw new Error('Tidak ada data attendance untuk disimpan');
      }

      // Prepare data for insertion with new fields
      const attendanceRecords = Object.values(attendanceData[classNum]).map(student => ({
        tanggal: date,
        nisn: student.nisn,
      ...
      const jenisText = getJenisPresensi() === 'kelas' ? 'Kelas' : 'Mata Pelajaran';
      showToast(`Presensi ${jenisText} ${classNum} tanggal ${date} berhasil disimpan!`);
  ...
  }, [attendanceData, showToast, loadAttendanceForDate, currentUser.username, getJenisPresensi]);
  ...
  const saveAttendance = useCallback(async () => {
    ...
    try {
      ...
      if (exists) {
        const jenisText = getJenisPresensi() === 'kelas' ? 'Kelas' : 'Mata Pelajaran';
        setModalMessage(`Data kehadiran untuk ${jenisText} ${activeClass} tanggal ${attendanceDate} sudah ada. Apakah Anda yakin ingin menimpa data yang sudah ada?`);
        setModalAction(() => () => saveAttendanceData(activeClass, attendanceDate));
        setShowModal(true);
      } else {
  ...
  }, [activeClass, attendanceDate, checkClassAccess, saving, showToast, checkExistingAttendance, saveAttendanceData, getJenisPresensi]);

  // Generate rekap data from database - Modified to include guru_input filter
  const generateRekapData = useCallback(async (classNum, month, year) => {
    try {
      ...
      // Modified query to filter by guru_input
      ...
      const rekapData = students.map(student => {
        const studentRecords = attendanceRecords.filter(record => record.nisn === student.nisn);
        ...
        const counts = {
          ...
          alpa: studentRecords.filter(r => r.status === 'Alpa' || r.status === 'Alfa').length
        };

        const totalDays = studentRecords.length;
        const percentage = totalDays > 0 ? Math.round((counts.hadir / totalDays) * 100) : 100;
  ...
  const showRekap = useCallback(async () => {
    ...
    const rekapData = await generateRekapData(activeClass, currentMonth.toString(), currentYear.toString());
    setRekapData(rekapData);
  }, [activeClass, generateRekapData, getJenisPresensi]);

  // Handle refresh data untuk RecapModal
  const handleRekapRefresh = useCallback(async (month, year) => {
    const rekapData = await generateRekapData(activeClass, month.toString(), year.toString());
    setRekapData(rekapData);
  }, [activeClass, generateRekapData]);

  // Export attendance function dengan ExcelJS
  const exportAttendance = useCallback(async (month, year) => {
    try {
    ...
    return (
      <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 bg-gray-50 min-h-screen">
        <div className="flex items-center justify-center py-12">
          ...
          <span className="text-gray-600 text-base sm:text-lg">Memuat data siswa...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
        <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
          <div className="flex items-center gap-3 lg:flex-shrink-0">
            <label className="font-semibold text-gray-700 whitespace-nowrap text-sm sm:text-base">Tanggal:</label>
            <input 
              type="date" 
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 flex-1">
            <button 
              ...
              disabled={!studentsData[activeClass] || studentsData[activeClass].length === 0 || saving}
            >
            ...
            <button 
              ...
              onClick={showRekap}
              disabled={!studentsData[activeClass] || studentsData[activeClass].length === 0}
            >
              <Calendar size={14} />
        ...
        <div className="sm:hidden space-y-3">
          {filteredStudents.length === 0 ? (
            ...
            filteredStudents.map((student, index) => {
              ...
              const attendance = attendanceData[activeClass]?.[originalIndex] || { status: 'Hadir', note: '' };
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-gray-100 border-b border-gray-200">
                  <th className="px-2 sm:px-4 py-3 sm:py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">No</th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">NISN</th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Siswa</th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden lg:table-cell">Jenis Kelamin</th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Status Kehadiran</th>
                  <th className="px-2 sm:px-4 py-3 sm:py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider hidden xl:table-cell">Keterangan</th>
                </tr>
              </thead>
                  ...
                  filteredStudents.map((student, index) => {
                    ...
                    return (
                      <tr key={originalIndex} className="hover:bg-gray-50 transition-colors">
                        <td className="px-2 sm:px-4 py-3 sm:py-4 text-xs sm:text-sm font-semibold text-gray-900">{index + 1}</td>
                        <td className="px-2 sm:px-4 py-3 sm:py-4 text-xs sm:text-sm font-medium text-gray-900">{student.nisn}</td>
