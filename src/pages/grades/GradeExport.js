// src/pages/grades/Export.js
import ExcelJS from "exceljs";
import { supabase } from "../../supabaseClient"; // ✅ DIPERBAIKI
import {
  Download,
  Upload,
  Loader,
...
export const ImportModal = ({
  ...
  const downloadTemplate = async () => {
    try {
      ...
      a.href = url;
      a.download = `Template_Nilai_${selectedSubject.replace(
        /\s+/g,
        "_"
      )}_Kelas_${selectedClass}.xlsx`;
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      alert(
        `✓ Berhasil import ${validData.length} data siswa!\nTotal nilai: ${dataToImport.length}`
      );
      handleClose();
      onImportSuccess();
    } catch (error) {
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-700 mb-4">
                      ...
                      Isi nilai pada kolom yang tersedia (NH-1 s/d UAS).
                    </p>
                    <button
...
export const exportToExcel = async ({
  ...
  try {
    ...
    const filename = `Nilai_${selectedSubject.replace(
      /\s+/g,
      "_"
    )}_Kelas_${selectedClass}.xlsx`;
...
const exportToExcelDefault = exportToExcel;
