    ...
    try {
      ...
      dataToSave.forEach((student) => {
        ...
        if (student.isExisting) {
          updateData.push(payload); // 👈 Data yang SUDAH ADA (edit)
        } else {
          newData.push(payload); // 👈 Data BARU (insert)
        }
      });
      ...
      // 🔥 HANDLE INSERT (Data Baru)
      if (newData.length > 0) {
        const insertResult = await saveWithSync("grades", newData);
      ...
      // 🔥 HANDLE UPDATE (Data Existing) - LOOP SATU-SATU
      if (updateData.length > 0) {
        for (const data of updateData) {
          try {
            // ⚠️ CRITICAL: Update berdasarkan COMPOSITE KEY
            const { data: updated, error } = await supabase
              ...
              .eq("nisn", data.nisn)
              .eq("kelas", data.kelas)
              .eq("mata_pelajaran", data.mata_pelajaran)
              .eq("jenis_nilai", data.jenis_nilai)
              .select();
            ...
            if (updated && updated.length > 0) {
              successCount++;
              console.log(
                `✅ Updated: ${data.nama_siswa} - ${data.jenis_nilai}`
              );
            } else {
              console.warn(`⚠️ No record updated for ${data.nama_siswa}`);
              failCount++;
            }
          } catch (error) {
            console.error(`❌ Update failed for ${data.nama_siswa}:`, error);
            failCount++;
          }
        }
      }
      ...
      if (successCount > 0 && failCount === 0) {
        showMessage(
          `✅ ${successCount} nilai berhasil disimpan/diupdate!`,
          "success"
        );
