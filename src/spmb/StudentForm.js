...
const StudentForm = ({ editingStudent, setEditingStudent, students, onSaveStudent, isLoading }) => {
  ...
  const refs = {
    nama: useRef(), jenis_kelamin: useRef(), tempat_lahir: useRef(),
  ...
  useEffect(() => {
    if (editingStudent) {
      setFormData({
        ...
        asal_sekolah: editingStudent.asal_tk || editingStudent.asal_sekolah || "",
  ...
  const validateStudentInput = useCallback((student) => {
    ...
    if (!/^[a-zA-Z\s]+$/.test(student.nama)) errors.push("Nama hanya boleh mengandung huruf dan spasi");
  ...
  const checkForDuplicates = useCallback((newName, existingStudents, excludeId = null) => {
    ...
    return existingStudents
      ...
      .map(student => ({
        student,
        similarity: calculateSimilarity(newName, student.nama_lengkap || student.nama || "")
      }))
  ...
  const handleSubmit = async (e) => {
    ...
    const duplicates = checkForDuplicates(studentData.nama, students, editingStudent?.id || null);
      ...
      {showDuplicateWarning && duplicateInfo && (
        <div className="bg-gradient-to-r from-yellow-100 to-orange-100 border-2 border-yellow-300 rounded-xl p-4 mb-6 flex items-center gap-3 animate-slide-down">
          ...
          <span className="text-yellow-800 flex-1">
            Nama serupa "{duplicateInfo.student.nama_lengkap || duplicateInfo.student.nama}" sudah terdaftar! 
