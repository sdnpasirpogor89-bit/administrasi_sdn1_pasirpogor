...
import { Users, TrendingUp, School, Clock, Calendar, UserCheck, Activity } from 'lucide-react';
...
const Statistics = ({ students, totalStudents, maleStudents, femaleStudents, getCurrentAcademicYear }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (currentStep <= steps) {
        setAnimatedCounts({
          total: Math.min(Math.floor(increment.total * currentStep), totalStudents),
          male: Math.min(Math.floor(increment.male * currentStep), maleStudents),
          female: Math.min(Math.floor(increment.female * currentStep), femaleStudents)
        });
      } else {
        clearInterval(timer);
      }
  ...
  const topSchoolsData = getSchoolStats.slice(0, 5).map(stat => ({
    name: stat.school.length > 20 ? stat.school.substring(0, 20) + '...' : stat.school,
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white px-4 py-2 rounded-lg shadow-lg border border-gray-200">
          <p className="text-sm font-semibold text-gray-800">{payload[0].payload.fullName || payload[0].name}</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-6">
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-100 mb-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Dashboard Statistik SPMB</h1>
            <p className="text-gray-600">SDN 1 Pasir Pogor • Tahun Ajaran {getCurrentAcademicYear()}</p>
          </div>
          <div className="text-right">
            <div className="text-4xl font-bold text-blue-600">{animatedCounts.total}</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <StatCard 
          ...
          subtitle={`${totalStudents > 0 ? Math.round((maleStudents / totalStudents) * 100) : 0}% dari total`}
        ...
        <StatCard 
          ...
          subtitle={`${totalStudents > 0 ? Math.round((femaleStudents / totalStudents) * 100) : 0}% dari total`}
        ...
        <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-lg">
              ...
              <div>
                <div className="font-semibold text-gray-900 text-sm">{maleStudents} Laki-laki</div>
            ...
            <div className="flex items-center gap-3 p-3 bg-pink-50 rounded-lg">
              ...
              <div>
                <div className="font-semibold text-gray-900 text-sm">{femaleStudents} Perempuan</div>
                <div className="text-xs text-gray-600">
              ...
              <div className="flex items-center gap-4">
                <div className="relative">
                  ...
                  <div className={`absolute -top-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${
                    student.jenis_kelamin === 'Laki-laki' ? 'bg-blue-500' : 'bg-pink-500'
                  }`}></div>
                </div>
                <div>
                  <div className="font-semibold text-gray-900 text-sm">{student.nama_lengkap}</div>
                  <div className="text-xs text-gray-500 flex items-center gap-1">
                    <School className="w-3 h-3" />
                    {student.asal_tk || student.asal_sekolah || 'Tidak ada data'}
                  </div>
                </div>
              </div>
              <div className="text-right">
                <div className="text-xs font-medium text-blue-600">
                  {student.tanggal_daftar ? new Date(student.tanggal_daftar).toLocaleDateString('id-ID', {
                    day: 'numeric',
                    month: 'short',
                    year: 'numeric'
                  }) : 'Baru'}
      ...
      <div className="mt-6 text-center text-sm text-gray-500">
        <p>Statistik diperbarui secara real-time • SDN 1 Pasir Pogor © {new Date().getFullYear()}</p>
      </div>
    </div>
  );
};
