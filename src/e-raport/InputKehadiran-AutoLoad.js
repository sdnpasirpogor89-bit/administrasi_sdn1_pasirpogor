    ...
    for (const siswa of students) {
      const siswaAttendance =
        attendanceData?.filter((a) => {
          ...
          if (siswa.nisn === "147784237") {
            console.log("NAYLA Debug:", {
              tanggal_raw: a.tanggal,
              tanggal_type: typeof a.tanggal,
              tanggalStr: tanggalStr,
              start: periodeRange.start,
              end: periodeRange.end,
              pass:
                tanggalStr >= periodeRange.start &&
                tanggalStr <= periodeRange.end,
            });
          }
