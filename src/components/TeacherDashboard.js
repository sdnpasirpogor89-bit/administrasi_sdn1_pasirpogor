...
const StatsCard = ({
  ...
  return (
    <div
      className={`bg-white rounded-lg sm:rounded-xl p-3 sm:p-5 shadow-sm border-l-4 ${colorClasses[color]} hover:shadow-md transition-all duration-200 touch-manipulation`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div>
            <p className="text-xs sm:text-base font-semibold text-gray-700 truncate leading-tight">
              {title}
            </p>
            {subtitle && (
        ...
        <div className="flex-shrink-0">
          <Icon
            size={20}
            className={`sm:size-6 lg:size-7 ${iconColorClasses[color]}`}
          />
        </div>
      </div>
    </div>
    ...
    <div className="bg-white rounded-lg p-3 shadow-sm border border-gray-200">
      ...
      <div className="grid grid-cols-4 gap-1 text-center">
        <div className="bg-green-50 rounded p-1">
          <span className="block font-bold text-green-700 text-xs">
            {hadir}
          </span>
          <span className="text-xs text-green-600">Hadir</span>
        </div>
        <div className="bg-yellow-50 rounded p-1">
          <span className="block font-bold text-yellow-700 text-xs">
            {izin}
          </span>
    ...
    return (
      <div className="bg-white rounded-xl sm:rounded-2xl p-3 sm:p-6 shadow-lg border border-gray-100">
        ...
        <div className="text-center py-8">
          ...
          <p className="text-sm text-gray-400 mt-1">Silakan cek hari lainnya</p>
        </div>
      </div>
    );
  }
...
const TeacherDashboard = ({ userData }) => {
  ...
  const [dashboardData, setDashboardData] = useState({
    ...
    totalClasses: 0,
    tidakHadir: 0,
    todaySchedule: [], // NEW: Tambah state untuk jadwal hari ini
  });
  ...
  const fetchTodaySchedule = async () => {
    try {
      ...
      // Jika hari Sabtu/Minggu, tidak ada jadwal
  ...
  const fetchGuruKelasDashboardData = async () => {
    try {
      ...
      const tidakHadirCount =
        classAttendance.izin + classAttendance.sakit + classAttendance.alpa;
  ...
  const fetchGuruMapelDashboardData = async () => {
    try {
      ...
      setDashboardData((prev) => ({
        ...
        tidakHadir: totalStudentsCount - todayPresentCount,
        todaySchedule: todaySchedule,
      }));
    } catch (error) {
    ...
    return (
      <div className="space-y-4 sm:space-y-6 pb-20 sm:pb-0">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
          ...
          {/* NEW: Jadwal Hari Ini - GANTI Trend Kehadiran */}
          <TodayScheduleCard
  ...
  const renderGuruMapelDashboard = () => {
    ...
    return (
      <div className="space-y-4 sm:space-y-6 pb-20 sm:pb-0">
        ...
        {/* NEW: Jadwal Hari Ini untuk Guru Mapel */}
        <TodayScheduleCard
          schedule={dashboardData.todaySchedule}
          isMobile={isMobile}
        />
