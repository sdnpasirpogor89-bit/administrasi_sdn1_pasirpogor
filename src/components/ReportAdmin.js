...
const Report = ({ userData }) => {
    ...
    // Fetch data function with real database structure
    const fetchData = async () => {
        ...
        try {
            ...
            // Students query with role-based filtering
            ...
            // Get student NISNs for filtering
            ...
            if (userRole === 'guru_kelas') {
                ...
                // Guru mapel bisa lihat semua kelas tapi yang dia input
    ...
    useEffect(() => {
        fetchData();
    }, [selectedPeriod, userRole, userKelas]);

    // Enhanced filtering with mata pelajaran and jenis nilai
    const getFilteredData = () => {
        ...
        if (userRole === 'admin' && selectedClass) {
            ...
            filteredAttendance = attendance.filter(a => a.kelas === classInt);
            filteredNilai = nilai.filter(n => n.kelas === classInt);
        }
    ...
    // Enhanced Analytics with real database structure
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center h-screen bg-gray-50">
            ...
            <p className="text-gray-500 text-sm mt-1">Memproses {selectedPeriod === '1year' ? '1 tahun' : selectedPeriod === '6months' ? '6 bulan' : selectedPeriod === '3months' ? '3 bulan' : '1 bulan'} terakhir</p>
        </div>
    );
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-500">
                                    {userRole === 'admin' ? 'Analisis performa seluruh sekolah' : 
                                     ...
                                     'Analisis mata pelajaran yang diampu'} • {periodOptions.find(p => p.value === selectedPeriod)?.label}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div>
                                ...
                                <div className="bg-gray-50 rounded-xl p-6 mt-6">
                                    <h4 className="font-semibold text-gray-900 mb-4">
                                        Ringkasan Laporan {periodOptions.find(p => p.value === selectedPeriod)?.label}
                                    </h4>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div className="flex gap-3">
                        <button 
                            onClick={() => alert("Export Excel akan navigasi ke tombol Export Excel pada halaman Kehadiran")}
                        ...
                        <button 
                            onClick={() => alert("Export Nilai akan navigasi ke tombol export pada halamanan Nilai Akademik")}
