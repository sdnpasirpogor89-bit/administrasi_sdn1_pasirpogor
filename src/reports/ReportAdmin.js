...
const ReportAdmin = ({ user }) => {
  ...
  const currentMenu = useMemo(
    () => reportMenus.find((m) => m.type === filters.type),
    [reportMenus, filters.type]
  );

  const hasValidData = data && data.length > 0;
  const showEmptyState = !loading && !error && !hasValidData;
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Statistics Cards - Only show when NOT loading */}
        {!loading && (
          <div className="mb-6">
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">
              Data {currentMenu?.title || "Laporan"}
            </h2>
          </div>
