...
// MASTER VALIDATOR - Single Source of Truth untuk semua validasi presensi (SD)
...
const SCHOOL_COORDS = {
  lat: -6.982349, // Koordinat sekolah
  lng: 107.393952,
};

const SCHOOL_RADIUS = 10000; // 200 meter radius
...
export const validateAttendance = async (options = {}) => {
  ...
  // ========================================
  // STEP 1: Validasi Waktu Operational (07:00-13:00)
  // ========================================
...
/**
 * Validasi waktu operational sekolah (07:00-13:00)
 */
const validateOperationalTime = () => {
  // 🚀 SKIP VALIDATION SAAT DEV MODE (untuk testing)
  if (DEV_MODE) {
    console.log("⚠️ DEV MODE ACTIVE: Time validation bypassed");
    return {
      allowed: true,
      currentTime: new Date().toTimeString().slice(0, 5),
      message: "⚠️ DEV MODE: Waktu presensi valid (bypassed for testing)",
      devMode: true,
    };
  }
...
export const validateTeacherSchedule = async (userId) => {
  try {
    ...
    // Query ke class_schedules dengan teacher_id = user.id
